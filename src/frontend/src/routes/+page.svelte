<script lang="ts">
  ...
  import { AssociatedFetchError, authUser, NotYetAssociatedError } from "$lib/stores/Auth";
    ...
    return async () => {
      try {
        ...
        if (submitType === IdentitySubmits.SIGN_UP) {
          toastStore.trigger({
            message: "This Internet Identity is already associated with a user. Logged in.",
        ...
        if (error instanceof NotYetAssociatedError) {
          ...
          if (submitType === IdentitySubmits.LOG_IN) {
            toastStore.trigger({
              message: "This Internet Identity is not yet associated with a user. Register.",
              background: "variant-filled-warning",
              classes: "rounded-2xl, font-semibold",
            });
          }
          ...
          toastStore.trigger({
            message: "Unable to fetch user associated with the Identity. Check your connection.",
            background: "variant-filled-warning",
            classes: "rounded-2xl, font-semibold",
          });
        } else {
    ...
    <div class="xl:flex mt-12">
      <div class="xl:w-1/2 flex flex-col xl:pr-5">
        <h1 class="h1 text-4xl text-center mb-10">
          <span class="bg-gradient-to-br from-blue-600 to-cyan-300 bg-clip-text text-transparent box-decoration-clone">
            Anonymous user
          </span>
        </h1>
        <div class="xl:ml-8">
        ...
        <div class="mt-12">
          ...
          <div class="mx-auto w-full xl:px-6 text-center mt-4">
            <button
              ...
              disabled={disabled}
              on:click={() => goto(getPath(ROUTES.SIGN_IN, ROUTES.REGISTER_ANONYMOUS))}
            >
      ...
      <div class="xl:w-1/2 flex flex-col xl:pl-5 mt-8 xl:mt-0">
        <div class="text-4xl text-center mb-10 font-extrabold">
          <div class="bg-gradient-to-br from-pink-600 to-violet-500 bg-clip-text text-transparent box-decoration-clone flex flex-col xl:flex-row justify-center content-center">
            <div class="mr-2">Nostric user</div>
            <div class="badge variant-soft uppercase rounded-2xl mt-4 xl:mt-0">recommended</div>
          </div>
        </div>
        <div class="xl:ml-8">
          <ul class="font-semibold list-disc ml-4">
            <li>fully <a href="somelink" target="_blank" class="underline">featured</a> Nostric client</li>
            <li>keys securely <a href="somelink" target="_blank" class="underline">encrypted in an ICP canister</a></li>
            <li>best user experience</li>
            <li>possibility to log in using <a href="somelink" target="_blank" class="underline">Internet Identity</a></li>
          </ul>
        </div>
        <div class="mt-12">
          <form method="POST" use:enhance={({cancel}) => onSubmit(IdentitySubmits.LOG_IN, cancel)} class="mx-auto w-full xl:px-6 text-center">
            <button
              type="submit"
          ...
          <form method="POST" use:enhance={({cancel}) => onSubmit(IdentitySubmits.SIGN_UP, cancel)} class="mx-auto w-full xl:px-6 text-center mt-4">
            <button
              type="submit"
              class="btn variant-filled rounded-2xl w-full font-semibold"
              disabled={disabled}
