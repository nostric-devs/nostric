...
export class NostrHandler extends EventEmitter {
  ...
  public async addSubscription(filter : NDKFilter) : Promise<void> {
    ...
    let subscription : NDKSubscription = this.nostrKit.subscribe(filter, options);
   ...
   */
  public async generatePublicKeyFromPrivateKey(privateKey : string) : Promise<string> {
   ...
   */
  public async fetchUserProfileByPublicKey(publicKey : string) : Promise<NDKUser> {
  ...
  public async searchByQuery(query : string) : Promise<UsersObject> {
    ...
    for (const event of events) {
      ...
      if (
        ...
        pubkey.includes(query) || user.npub.includes(query)
      ) {
        users[pubkey] = user;
      }
    }
