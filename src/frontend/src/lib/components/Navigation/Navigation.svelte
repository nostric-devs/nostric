<script lang="ts">
  ...
  import { Home, Search, User, Settings, Bookmark, Bell } from "svelte-feathers";
...
<nav class="list-nav xl:px-4 py-4">
  ...
  <ul class="pl-8 pt-2">
    <a
      ...
      class:bg-primary-active-token={isActive(get_path(ROUTES.SETTINGS, ROUTES.PROFILE))}
      class="capitalize"
    >
      <span class="lg:hidden xl:block ml-3">{ROUTES.PROFILE}</span>
    </a>
    <a
      ...
      class:bg-primary-active-token={isActive(get_path(ROUTES.SETTINGS, ROUTES.RELAYS))}
      class="capitalize"
    >
    ...
    <a
      ...
      class:bg-primary-active-token={isActive(get_path(ROUTES.SETTINGS, ROUTES.PRO))}
      class="capitalize"
    >
