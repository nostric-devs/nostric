...
function getEvents() {
  ...
  const add = (event: NDKEvent) => update((events: NDKEvent[]) => {
    if (!events.find((past_event : NDKEvent) : boolean => past_event.id === event.id)) {
  ...
  const filterTextEventsByPublicKey = (publicKey : string) : NDKEvent[] => {
    return get(events).filter(
      (event: NDKEvent) : boolean => event.kind === NDKKind.Text && event.pubkey === publicKey
    );
  }
  ...
  const getEventReactions = (mainEvent : NDKEvent) : NDKEvent[] => {
    return get(events).filter(
      (event : NDKEvent) : boolean =>
        ...
        event.tags.find((tag : NDKTag) => tag[0] === "e" && tag[1] === mainEvent.id) !== undefined
