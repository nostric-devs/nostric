...
export type AuthUser = {
  ...
  isProAuthenticated: () => boolean; setAnonymous: () => void;
...
export function getAuthUser() : AuthUser {
  ...
  const authState : Writable<AuthState> = writable<AuthState>(AuthStates.ANONYMOUS);
  ...
  const isNostrAuthenticated = () : boolean => get(authState) === AuthStates.NOSTR_AUTHENTICATED;
  const isNostricAuthenticated = () : boolean => get(authState) === AuthStates.NOSTRIC_AUTHENTICATED;
  const setNostricAuthenticated = () : void => set(AuthStates.NOSTRIC_AUTHENTICATED);
  const isProAuthenticated = () : boolean => get(authState) === AuthStates.PRO_AUTHENTICATED;
